...
-- Fjord - "Nordic Twilight" Edition
-- Blue-gray backgrounds with aurora accents
...
local config = {
  transparent = vim.g.fjord_transparent or false,
}

--------------------------------------------------------------------------------
-- COLOR PALETTE
-- "Nordic Twilight" Palette: Cold blue-gray with aurora colors
--------------------------------------------------------------------------------

local colors = {
  -- The Foundation (Fjord Blue-Gray Backgrounds)
  black = '#1e2430',
  bg_dark = '#1a1f2a',
  bg_mid = '#242b38',
  bg_light = '#2a3240',
  ...
  -- The Foregrounds (Frost Tones)
  fg_bright = '#e5e9f0',
  fg_normal = '#d8dee9',
  fg_muted = '#a3adc2',

  -- The Accents (Aurora Colors)
  glacier = '#88c0d0',
  aurora_green = '#a3be8c',
...
local function get_palette()
  ...
  return {
    ...
    -- UI foregrounds
    fg = colors.fg_normal, -- Base text is frost white
    fg_dim = colors.stone, -- Dim text is stone gray
    fg_inactive = colors.graphite, -- Inactive is darker graphite

    -- Diff backgrounds (Nordic subtle tints)
    bg_diff_add = '#1f2d26',
    bg_diff_change = '#242a38',
    bg_diff_delete = '#2d2226',
    bg_diff_text = '#323a4a',

    -- Syntax semantics (Nordic with Aurora Accents)
    -- The rule: Keywords bright frost, Functions aurora green, Types glacier blue.

    keyword = colors.fg_bright, -- Keywords are brightest frost (Bold)
    func = colors.aurora_green, -- Functions get aurora green (Bold)

    string = colors.aurora_green, -- Strings get aurora green
    constant = colors.fg_muted, -- Constants in muted gray

    type = colors.glacier, -- Types get glacier blue for distinction
    variable = colors.fg_normal, -- Variables are standard frost
    property = colors.fg_normal, -- Properties are standard frost

    operator = colors.graphite, -- Operators recede
    comment = colors.stone, -- Comments in stone gray
    punctuation = colors.graphite, -- Punctuation recedes

    tag = colors.fg_bright, -- HTML tags pop
    attribute = colors.aurora_green, -- Attributes get aurora green
    ...
    git_change = colors.glacier,
    git_delete = colors.error_soft,

    -- Special
    match = colors.aurora_yellow, -- Search matches in aurora yellow
    link = colors.glacier, -- Links in glacier blue

    -- Additional semantic colors for the new palette
    white = colors.fg_bright, -- Brightest color for emphasis
    steel = colors.frost, -- Accent color for UI elements
...
local function set_terminal_colors(_)
  ...
  vim.g.terminal_color_4 = colors.frost -- blue
  vim.g.terminal_color_5 = colors.heather -- magenta
  vim.g.terminal_color_6 = colors.glacier -- cyan
  ...
  vim.g.terminal_color_11 = colors.aurora_yellow -- bright yellow
  vim.g.terminal_color_12 = colors.frost -- bright blue
  vim.g.terminal_color_13 = colors.fg_bright -- bright magenta
  vim.g.terminal_color_14 = colors.glacier -- bright cyan
...
-- Usage: require('fjord').toggle_transparent()
return {
  toggle_transparent = function()
    config.transparent = not config.transparent
    vim.g.fjord_transparent = config.transparent
    setup()
    vim.notify('Fjord: Transparent = ' .. tostring(config.transparent), vim.log.levels.INFO)
  end,
  set_transparent = function(value)
    config.transparent = value
    vim.g.fjord_transparent = value
    setup()
  end,
}
