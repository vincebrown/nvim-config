...
-- Charcoal Whisper - "Earth & Sage" Edition
-- A warm, natural palette with earthy tones and sage green accents.
...
local config = {
  transparent = vim.g.charcoal_whisper_transparent or false,
}

--------------------------------------------------------------------------------
-- COLOR PALETTE
-- "Earth & Sage" Palette: Warm, natural tones inspired by everforest/nord.
--------------------------------------------------------------------------------

local colors = {
  -- The Foundation (Warm Dark Backgrounds)
  black = '#1e1e1e',        -- Deepest warm black
  bg_dark = '#252525',      -- Floats (warm charcoal)
  bg_mid = '#2d2d2d',       -- Cursorline (warm mid-tone)
  bg_light = '#343434',     -- Statusline (warm grey)
  bg_highlight = '#3a3a3a', -- Visual selection (warm highlight)
  bg_accent = '#404040',    -- Search/Highlights (warm accent)

  -- The Earth (Warm, Natural Foregrounds - Increased Contrast)
  fg_bright = '#eddfca',    -- Bright warm cream (Keywords - high contrast)
  fg_normal = '#c9b89a',    -- Warm tan (Base text - medium)
  fg_muted = '#9a8d7a',     -- Muted brown-tan (Secondary text - low)
  
  -- The Accents (Your preferred sage + variations)
  sage = '#b0c4b1',         -- Your favorite sage green (Primary - strings, data)
  seafoam = '#a0b5aa',      -- Darker seafoam (Secondary - links, info)
  stone = '#6a756a',        -- Stone grey-green (Comments, dim)
  
  -- Punctuation & Structure
  graphite = '#555555',     -- Dark grey (Punctuation/delimiters)

  -- Functional Colors (Warm, desaturated tones)
  error_soft = '#d08a8a',   -- Soft red
  warn_soft = '#d8c088',    -- Soft yellow/gold
  success_soft = '#a8b89f', -- Soft green
}
...
local function get_palette()
  ...
  return {
    ...
    -- UI foregrounds
    fg = colors.fg_normal,       -- Base text is warm beige
    fg_dim = colors.stone,       -- Dim text is stone grey-green
    fg_inactive = colors.graphite, -- Inactive is darker grey

    -- Diff backgrounds (Warm subtle tints)
    bg_diff_add = '#232822',
    bg_diff_change = '#242628',
    bg_diff_delete = '#2a2424',
    bg_diff_text = '#323232',

    -- Syntax semantics (Natural with Clear Differentiation)
    -- The rule: Keywords bright, Functions medium, Types distinct, Data gets sage.
    
    keyword = colors.fg_bright,  -- Keywords are brightest (Bold) - "private", "async"
    func = colors.sage,          -- Functions get sage green (Bold) - "getCachedPostalCodeCoordinates"
    
    string = colors.sage,        -- Strings get sage green
    constant = colors.fg_muted,  -- Constants in muted tan
    
    type = colors.seafoam,       -- Types get seafoam for distinction
    variable = colors.fg_normal, -- Variables are standard tan
    property = colors.fg_normal, -- Properties are standard tan
    
    operator = colors.graphite,  -- Operators recede
    comment = colors.stone,      -- Comments in stone grey-green
    punctuation = colors.graphite, -- Punctuation recedes
    
    tag = colors.fg_bright,      -- HTML tags pop
    attribute = colors.sage,     -- Attributes get sage
    ...
    -- Special
    match = colors.sage,         -- Search matches in sage
...
local function set_terminal_colors(_)
  ...
  vim.g.terminal_color_9 = colors.error_soft -- bright red
  vim.g.terminal_color_10 = colors.success_soft -- bright green
  vim.g.terminal_color_11 = colors.warn_soft -- bright yellow
...
local function setup()
  ...
  vim.g.colors_name = 'charcoal-whisper'
...
-- Usage: require('charcoal-whisper').toggle_transparent()
return {
  toggle_transparent = function()
    config.transparent = not config.transparent
    vim.g.charcoal_whisper_transparent = config.transparent
    setup()
    vim.notify('Charcoal Whisper: Transparent = ' .. tostring(config.transparent), vim.log.levels.INFO)
  end,
  set_transparent = function(value)
    config.transparent = value
    vim.g.charcoal_whisper_transparent = value
    setup()
  end,
}
