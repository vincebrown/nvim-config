...
-- Ember - "Glowing Coals" Edition
-- Deep warm blacks with firelight accents
...
local config = {
  transparent = vim.g.ember_transparent or false,
}

--------------------------------------------------------------------------------
-- COLOR PALETTE
-- "Glowing Coals" Palette: Ash black with firelight warmth
--------------------------------------------------------------------------------

local colors = {
  -- The Foundation (Ash Black Backgrounds)
  black = '#1a1614',
  bg_dark = '#161210',
  bg_mid = '#201c1a',
  bg_light = '#282320',
  ...
  -- The Foregrounds (Warm Cream Tones)
  fg_bright = '#e8d8c8',
  fg_normal = '#c9b89a',
  fg_muted = '#9a8a78',

  -- The Accents (Firelight Colors)
  ember = '#e8a87c',
  golden = '#c9a86c',
  smoke_green = '#a8c99d',
  coral = '#d49090',
...
local function get_palette()
  ...
  return {
    ...
    -- UI foregrounds
    fg = colors.fg_normal, -- Base text is warm cream
    fg_dim = colors.charcoal, -- Dim text is charcoal gray
    fg_inactive = colors.ash, -- Inactive is ash gray

    -- Diff backgrounds (Warm ember tints)
    bg_diff_add = '#232820',
    bg_diff_change = '#28251e',
    bg_diff_delete = '#2a201e',
    bg_diff_text = '#3a332e',

    -- Syntax semantics (Warm with Firelight Accents)
    -- The rule: Keywords bright cream, Functions smoke green, Types ember orange.

    keyword = colors.fg_bright, -- Keywords are brightest cream (Bold)
    func = colors.smoke_green, -- Functions get smoke green (Bold)

    string = colors.smoke_green, -- Strings get smoke green
    constant = colors.fg_muted, -- Constants in muted cream

    type = colors.ember, -- Types get ember orange for distinction
    variable = colors.fg_normal, -- Variables are standard cream
    property = colors.fg_normal, -- Properties are standard cream

    operator = colors.ash, -- Operators recede
    comment = colors.charcoal, -- Comments in charcoal gray
    punctuation = colors.ash, -- Punctuation recedes

    tag = colors.fg_bright, -- HTML tags pop
    attribute = colors.smoke_green, -- Attributes get smoke green
    ...
    git_change = colors.ember,
    git_delete = colors.error_soft,

    -- Special
    match = colors.golden, -- Search matches in golden
    link = colors.ember, -- Links in ember orange

    -- Additional semantic colors for the new palette
    white = colors.fg_bright, -- Brightest color for emphasis
    steel = colors.flame, -- Accent color for UI elements
...
local function set_terminal_colors(_)
  ...
  vim.g.terminal_color_4 = colors.flame -- blue
  vim.g.terminal_color_5 = colors.coral -- magenta
  vim.g.terminal_color_6 = colors.smoke_green -- cyan
  vim.g.terminal_color_7 = colors.fg_normal -- white
  vim.g.terminal_color_8 = colors.charcoal -- bright black
  vim.g.terminal_color_9 = colors.error_soft -- bright red
  vim.g.terminal_color_10 = colors.success_soft -- bright green
  vim.g.terminal_color_11 = colors.golden -- bright yellow
  vim.g.terminal_color_12 = colors.flame -- bright blue
  vim.g.terminal_color_13 = colors.fg_bright -- bright magenta
  vim.g.terminal_color_14 = colors.smoke_green -- bright cyan
...
-- Usage: require('ember').toggle_transparent()
return {
  toggle_transparent = function()
    config.transparent = not config.transparent
    vim.g.ember_transparent = config.transparent
    setup()
    vim.notify('Ember: Transparent = ' .. tostring(config.transparent), vim.log.levels.INFO)
  end,
  set_transparent = function(value)
    config.transparent = value
    vim.g.ember_transparent = value
    setup()
  end,
}
